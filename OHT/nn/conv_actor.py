...
class NeuralNetwork():
    ...
    def init_model(self, inp_size, layers, activation_fn, output_activation, optimizer):
        ...
        model.add(keras.layers.Conv2D(layers[0], (3,3), activation=self.ACTIVATION_FUNCTIONS[activation_fn]))
...
class Actor():
    ...
    def save(self, game_name, game, episode, num_games):
        ...
        if game_name == 'nim':
            ...
            filename = f"{game.board.board_size}/new_cnn/Hex-{game.board.board_size}x{game.board.board_size}-at-{episode}-of-{num_games}-episodes-{datetime.datetime.strftime(datetime.datetime.now(), '%d-%m-%Y-%H-%M')}"
