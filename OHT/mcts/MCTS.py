...
class MCTS:
    def __init__(self, root, player, game, c):
    ...
    def rollout(self, state, path):
        ...
        while state.winner == -1:
            # move = self.get_move_default_policy(state.to_list_representation(), moves, state.LEGAL_MOVES)
    ...
    def string_state_to_tensor(self, state):
        return np.array([float(i) for i in state]).reshape(1, len(state))

    def get_move_default_policy(self, state, legal_moves, game_legal_moves):
        distribution = self.ANET(self.string_state_to_tensor(state)).numpy().tolist()[0]
